...
export default function DataFreshnessBar() {
  ...
  const { data: status } = useApi(fetchStatus, null, 30_000)
  ...
  const sources = [
    {
      label: "Bluesky",
      value: status?.last_bluesky,
      color: "bg-sky-400"
    },
    {
      label: "Trends",
      value: status?.last_google_trends,
      color: "bg-emerald-400"
    },
    {
      label: "Odds",
      value: status?.last_odds,
      color: "bg-amber-400"
    },
  ]
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-1.5 flex items-center gap-4 sm:gap-6 overflow-x-auto">
        ...
        {status && (
          <span className="text-white/20 text-xs ml-auto shrink-0">
            {(status?.total_snapshots ?? 0).toLocaleString()} snapshots stored
          </span>
        )}
