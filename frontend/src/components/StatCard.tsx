...
export default function StatCard({ label, value, sub, accent = "green", loading, labelNote }: Props) {
  return (
    <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex flex-col gap-1">
      {labelNote ? (
        <span className="relative group inline-flex items-start gap-0.5 w-fit max-w-full">
          <span className="text-white/40 text-[9px] sm:text-xs uppercase tracking-wide sm:tracking-widest leading-snug">{label}</span>
      ...
      {loading ? (
        ...
        <div
          className={`text-sm sm:text-2xl font-bold ${accents[accent]} flex items-center gap-1.5 min-w-0 leading-tight [&_span]:truncate`}
        >
          {value}
        </div>
      )}
