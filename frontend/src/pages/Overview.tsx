...
export default function Overview() {
  const { data: leaderboard, loading } = useApi(() => api.leaderboard(), [], 60_000)
  const { data: spikes } = useApi(() => api.spikes(), [], 30_000)

  const totalMentions = leaderboard.reduce((s: number, t: any) => s + (t.mentions || 0), 0)
  const avgPositivity = leaderboard.length
    ? Math.round(leaderboard.reduce((s: number, t: any) => s + (t.positive || 0), 0) / leaderboard.length * 100)
    : 0
  const topByOdds = [...leaderboard].sort((a: any, b: any) => (b.win_probability || 0) - (a.win_probability || 0))[0]
  const fanFavourite = [...leaderboard].sort((a: any, b: any) => (b.compound || 0) - (a.compound || 0))[0]
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard
          ...
          sub={topByOdds ? `${(topByOdds.win_probability * 100).toFixed(1)}% chance` : "odds loading"}
          accent="amber"
          loading={loading}
        />
        <StatCard
      ...
      {spikes.length > 0 && (
        <div className="space-y-2">
          ...
          {spikes.slice(0, 3).map((s: any, i: number) => (
            <div key={i} className="flex items-center gap-3 bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-3">
              <span className="w-2 h-2 rounded-full bg-red-400 animate-pulse shrink-0" />
              <TeamFlag team={s.team} size="sm" />
              <span className="text-white text-sm font-medium">{s.team}</span>
              <span className="text-red-400 text-xs">+{Math.round((s.spike_multiplier - 1) * 100)}% mentions</span>
              {s.inferred_trigger && (
                <span className="text-white/40 text-xs ml-auto truncate max-w-xs">{s.inferred_trigger}</span>
              )}
            </div>
          ))}
        </div>
          ...
          <div className="space-y-1.5">
            {leaderboard.map((team: any, i: number) => {
              ...
              return (
                <div
                  key={team.team}
                  className="flex items-center gap-3 bg-white/5 hover:bg-white/8 border border-white/5 hover:border-white/10 rounded-xl px-4 py-3 transition-all group"
                >
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full border hidden md:inline ${CONF_COLORS[conf]}`}>
                    {conf}
                  </span>
                  <div className="flex-1 min-w-0 hidden md:block">
                    <SentimentBar positive={team.positive} negative={team.negative} size="sm" />
                  </div>
                  <span className="text-white/40 text-xs w-20 text-right shrink-0">
                    {(team.mentions || 0).toLocaleString()} posts
                  </span>
                  ...
                  <span className={`text-xs w-12 text-right shrink-0 font-mono ${
                    (team.compound || 0) >= 0.1 ? "text-emerald-400" :
                    (team.compound || 0) <= -0.1 ? "text-red-400" : "text-white/40"
                  }`}>
                    {team.compound >= 0 ? "+" : ""}{(team.compound || 0).toFixed(2)}
                  </span>
                </div>
              )
            })}
