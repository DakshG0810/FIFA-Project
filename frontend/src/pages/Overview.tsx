...
export default function Overview() {
  ...
  const {
    data: leaderboard,
    loading,
  } = useApi(fetchLeaderboard, [], 60000)

  const {
    data: spikes,
  } = useApi(fetchSpikes, [], 30000)
  ...
  const topByOdds = [...leaderboard].sort(
    (a: any, b: any) =>
      (b.win_probability || 0) -
      (a.win_probability || 0)
  )[0]

  const fanFavourite = [...leaderboard].sort(
    (a: any, b: any) =>
      (b.compound || 0) -
      (a.compound || 0)
  )[0]
    ...
    <div className="space-y-8">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-950 via-black to-black border border-emerald-500/20 p-8">
        ...
        <div className="relative">
          ...
          <p className="text-white/40 text-sm max-w-xl">
            Real-time fan sentiment from Bluesky ·
            Search buzz from Google Trends ·
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard
          ...
          value={
            topByOdds
              ? `${getFlag(topByOdds.team)} ${topByOdds.team}`
              : "—"
          }
          sub={
            topByOdds
              ? `${(
                  topByOdds.win_probability * 100
                ).toFixed(1)}% chance`
              : "odds loading"
          }
          accent="amber"
        ...
        <StatCard
          ...
          value={
            fanFavourite
              ? `${getFlag(fanFavourite.team)} ${fanFavourite.team}`
              : "—"
          }
      ...
      {spikes.length > 0 && (
        ...
        <div className="space-y-2">
          ...
          <h2 className="text-white/50 text-xs uppercase tracking-widest">
            Viral spikes detected
          </h2>

          {spikes.slice(0, 3).map(
            (s: any, i: number) => (
          ...
          <div className="space-y-1.5">
            ...
            {leaderboard.map(
              (team: any, i: number) => {
                ...
                const conf =
                  CONF_MAP[team.team] || "UEFA"

                const pct =
                  team.win_probability
                    ? `${(
                        team.win_probability * 100
                      ).toFixed(1)}%`
                    : "—"
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full border hidden md:inline ${CONF_COLORS[conf]}`}
                    >
                      {conf}
                    </span>
                    ...
                    <span className="text-amber-400 text-xs w-12 text-right shrink-0 font-mono">
                      {pct}
                    </span>
