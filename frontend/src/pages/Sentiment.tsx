        ...
        <div className="md:col-span-3 space-y-1.5 min-w-0">
          {loading ? (
            ...
            <>
              ...
              <div className="sm:hidden flex items-center gap-2 px-3 py-2 text-[10px] uppercase tracking-wider text-white/35 border-b border-white/10">
                <span className="w-5 text-right shrink-0">#</span>
              ...
              {sorted.map((team, index) => {
                ...
                return (
                  <div key={team.team}>
                    <button
                      type="button"
                      onClick={() => setSelected(isSelected ? null : team.team)}
                      className={`w-full flex items-center gap-2 sm:gap-3 rounded-xl px-3 sm:px-4 py-3 border transition-all text-left ${rowClass}`}
                    >
                      <span className="text-white/25 text-xs font-mono w-5 text-right shrink-0">
                        {index + 1}
                      </span>
                      <TeamFlagImg team={team.team} size={18} />
                      <span className="text-white text-sm font-medium flex-1 min-w-0 truncate sm:w-28 sm:flex-none sm:shrink-0">
                        {team.team}
                      </span>
                      ...
                      <span className="text-white text-xs font-mono w-12 sm:w-16 text-right shrink-0">
                        {(team.mentions || 0).toLocaleString()}
                      </span>
                      <span
                        className={`text-xs font-mono w-12 sm:w-14 text-right shrink-0 ${
                    ...
                    {isSelected && (
                      <div className="md:hidden mt-2 mb-1">
                        <TeamHistoryPanel team={team.team} chartData={chartData} loading={histLoading} />
                      </div>
                    )}
                  </div>
                )
        ...
        <div className="hidden md:block md:col-span-2">
          {selected ? (
            <div className="sticky top-20">
              <TeamHistoryPanel team={selected} chartData={chartData} loading={histLoading} />
            </div>
          ) : (
