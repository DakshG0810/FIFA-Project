...
type Sort = "compound" | "mentions" | "positive" | "negative"

export default function Sentiment() {
  const { data, loading } = useApi(() => api.sentiment(48), [], 60_000)
  const [sort, setSort]   = useState<Sort>("mentions")
  const [selected, setSelected] = useState<string | null>(null)
  const { data: history, loading: histLoading } = useApi(
    () => selected ? api.teamHistory(selected) : Promise.resolve([]),
    [],
    120_000
  )

  const sorted = [...data].sort((a: any, b: any) => {
    if (sort === "compound")  return (b.compound || 0) - (a.compound || 0)
    if (sort === "positive")  return (b.positive || 0) - (a.positive || 0)
    if (sort === "negative")  return (b.negative || 0) - (a.negative || 0)
    return (b.mentions || 0) - (a.mentions || 0)
  })

  // Format history for chart
  const chartData = history.map((h: any) => ({
    time: new Date(h.captured_at).toLocaleDateString("en", { month: "short", day: "numeric", hour: "2-digit" }),
    compound: parseFloat((h.compound || 0).toFixed(3)),
    mentions: h.mentions || 0,
  }))

  const sortButtons: { key: Sort; label: string }[] = [
    { key: "mentions",  label: "Most discussed" },
    { key: "compound",  label: "Most positive"  },
    { key: "negative",  label: "Most negative"  },
    { key: "positive",  label: "Highest positive %" },
  ]
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-white/40 text-sm">Fan sentiment from Bluesky posts · Updated every 30 minutes</p>
      </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex gap-3 mt-1">
                    <span className="text-emerald-400 text-xs">{pos}% pos</span>
                    <span className="text-red-400 text-xs">{neg}% neg</span>
                  </div>
                </div>
        ...
        <div className="md:col-span-2">
          {selected ? (
            <div className="bg-white/5 border border-white/10 rounded-2xl p-5 sticky top-20 space-y-4">
              ...
              <p className="text-white/30 text-xs">Sentiment over time</p>
              {histLoading ? (
                ...
                <div className="h-40 flex items-center justify-center text-white/20 text-sm">
                  No history yet — check back after first collection
                </div>
              )}
            </div>
          ) : (
            <div className="bg-white/5 border border-white/10 rounded-2xl p-5 flex items-center justify-center h-40">
              <span className="text-white/20 text-sm">Select a team to see their sentiment history</span>
            </div>
          )}
        </div>
      </div>
